...
# ==== MONTHLY CRSP SETUP ==== 
crspm = read_fst(paste0(pathThesisData,'raw/m_crsp_raw.fst'))
...
gc()
crspminfo <- crspm %>%
  select(permno, yyyymm, prc, exchcd, me, shrcd, siccd) %>%
  arrange(permno, yyyymm)
...
# write to disk
write_fst(crspmret, paste0(pathThesisData,'interim/crspmret.fst'))
write_fst(crspminfo, paste0(pathThesisData,'interim/crspminfo.fst'))

write_csv(crspmret, paste0(pathThesisData,'external/crspmret.csv'))
write_csv(crspminfo, paste0(pathThesisData, 'external/crspminfo.csv'))
